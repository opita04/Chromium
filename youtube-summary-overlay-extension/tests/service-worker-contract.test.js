...
async function testSummarizeRoutesThroughLocalJobApi() {
  const worker = createServiceWorker({ fetchHandler: async (url, options = {}) => {
    ...
    if (url.endsWith('/api/youtube-summary/jobs')) { assert.equal(options.method, 'POST'); assert.equal(options.headers['Content-Type'], 'application/json'); assertLocalAuthHeader(options); const body = JSON.parse(options.body); assert.equal(body.video.videoId, 'abc123'); assert.equal(body.model, 'openrouter/free'); return jsonResponse({ ok: true, id: 'job-1', status: 'queued' }, true, 202); }
    if (url.endsWith('/api/youtube-summary/jobs/job-1')) { assert.equal(options.method, 'GET'); return jsonResponse({ ok: true, id: 'job-1', status: 'succeeded', result: { ok: true, markdown: '# Local Summary', path: '/tmp/local.md', category: 'General', model: 'openrouter/free' } }); }
    throw new Error('unexpected fetch ' + url);
  } });
  const result = await worker.send({ type: 'SUMMARIZE_AND_SAVE', video: { videoId: 'abc123', title: 'Example', transcript: 'Long transcript '.repeat(20) }, model: 'openrouter/free' });
...
function testSummaryDeadlineIsNinetySeconds() { const summaryTimeout = Number(contentScriptSource.match(/SUMMARY_RESPONSE_TIMEOUT_MS\s*=\s*(\d+)/)?.[1] || 0); const localJobTimeout = Number(scriptSource.match(/LOCAL_SUMMARY_JOB_TIMEOUT_MS\s*=\s*(\d+)/)?.[1] || 0); assert.equal(summaryTimeout, 90000, 'the visible summary deadline must stay at 90 seconds'); assert.ok(localJobTimeout < summaryTimeout, 'the background job deadline must finish before the visible deadline'); }

(async () => { await testSummarizeRoutesThroughLocalJobApi(); await testLocalAuthBootstrapUsesAllowlistedIdWhenRuntimeIdIsMissingOrSafariScoped(); await testSummarizeReturnsFailedLocalJobAsMessageResponse(); await testFindExistingRoutesThroughLocalService(); await testTranscriptFallbackRoutesThroughLocalService(); await testSaveMarkdownRoutesThroughLocalService(); await testLocalServiceHttpErrorsBecomeMessageErrors(); await testLocalServiceNetworkErrorsBecomeUnavailableMessage(); await testMutatingLocalAuthErrorsBecomeClearMessage(); await testMutatingRequestRetriesOnceAfterStaleLocalAuth(); await testLegacyAuthEndpointStillWorksForOldLocalServices(); await testActionClickInjectsYoutubeContentScriptWhenMissing(); testSummaryDeadlineIsNinetySeconds(); console.log('service-worker-contract.test.js passed'); })().catch((error) => { console.error(error); process.exit(1); });
